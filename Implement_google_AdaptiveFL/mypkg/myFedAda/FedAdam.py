...
# FedAvg K adjusted ver.
class MyFedAdam(FedOpt):
    '''
    Flower FedAdam: https://github.com/adap/flower/blob/main/src/py/flwr/server/strategy/fedadam.py
        eta (float, optional): [need tune] Server-side learning rate. Defaults to 1e-1. #
        eta_l (float, optional): [not used, need tune] Client-side learning rate. Defaults to 1e-1.
        beta_1 (float, optional): Momentum parameter. Defaults to 0.9.
        beta_2 (float, optional): Second moment parameter. Defaults to 0.99.
        tau (float, optional): [need tune] Controls the algorithm's degree of adaptability.
            Defaults to 1e-9.
    '''
